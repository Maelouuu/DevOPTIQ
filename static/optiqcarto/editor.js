...
async function importVSDX(file) {
  ...
  try {
    // spliceDecisions désactivé : insérer les losanges décoratifs dans le flux
    // (A→B devient A→D→B) rajoutait un nœud sur chaque chemin et COMPLEXIFIAIT les
    // flèches pour un gain faible. En reconstruction classique les tracés Visio sont
    ...
    render();
    // NB : en reconstruction classique les flèches ne sont PAS re-routées → les losanges
    // décoratifs restent posés sur leur flèche Visio d'origine, rien à re-seater.
