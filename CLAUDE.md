...
- **Flèches alignées DROITES** (`_straightenAlignedConnectors()`, appelé à l'import avant le polish) : une flèche que Visio a tracée tout droit (≤2 pts / colinéaire) entre deux formes verticalement (ou horizontalement) alignées mais légèrement décalées (centres à 10-24 px) devenait un ESCALIER car les deux ports tombaient à des X différents (> seuil « ligne droite » 4 px). On aligne les deux ports sur une coordonnée commune prise dans le recouvrement des formes → tracé rectiligne fidèle Visio. Garde-fous : uniquement connecteurs longs (>120 px) et formes vraiment superposées (≥45 %). Banc hard.vsdx : 3 verticales en escalier → 0/45.
