...
- **Losanges de décision** — DEUX cas selon la connectivité :
  ...
  - **Décoratifs (non connectés)** : dans Visio certains losanges sont posés « sur » un connecteur sans `<Connect>`. **`spliceDecisions` (ACTIVÉ à l'import)** les insère dans le flux : `A→B` devient `A→D→B` → ils deviennent de vrais nœuds connectés et bien placés. Les deux moitiés héritent du style (trait/pointillé) du connecteur d'origine (label une seule fois). Filet : `_seatFloatingDecisions()` (post-routage) repose sur la flèche la plus proche ceux qui n'ont pas pu être spliced.
...
- **Import VSDX** : à la reconstruction, l'utilisateur **choisit** (`_askVsdxLayoutMode`) entre **Agencement automatique** (`_computeAutoLayout()`, routage propre) et **Reconstruction classique** (fidèle Visio : les `customPath` exacts deviennent les `userPts` rendus). `spliceDecisions` est actif dans les deux cas.
