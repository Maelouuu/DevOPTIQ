...
- **Losanges de décision** — DEUX cas selon la connectivité :
  ...
  - **Décoratifs (non connectés)** : dans Visio certains losanges sont posés « sur » un connecteur sans `<Connect>`. **`spliceDecisions` est DÉSACTIVÉ** (opt-in seulement, cf. `vsdx_importer.js`) : l'insérer dans le flux (`A→B` → `A→D→B`) rajoutait un nœud sur chaque chemin et **complexifiait les flèches** (mesuré sur hard.vsdx : plus de coudes, flèches enmêlées) pour un gain faible. Comportement retenu : `_seatFloatingDecisions()` (post-routage) repose chaque losange décoratif sur la flèche la plus proche.
