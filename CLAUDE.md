...
- **Correction ciblée des erreurs (NOUVEAU)** — `_autoFixIssues()` dans editor.js, bouton **« Corriger les erreurs »** dans la pop-up Diagnostic carto (à côté d'« Agencement auto »). Contrairement à l'agencement auto (réorganise TOUTE la carto), la correction ciblée ne touche QUE les formes fautives relevées et laisse le reste tel quel. Deux familles corrigées : **hors-bande** (`outofband` → recentrage vertical dans la bande la plus proche via `_nearestBand`) et **chevauchement** (`overlap`, nouvelle détection dans `runCartoCheck` → `_findFreeSpot` déplace la forme fautive vers l'emplacement libre le plus proche, en restant dans sa bande). Seules les flèches rattachées à une forme déplacée voient leur routage réinitialisé (`userPts/customPath/…` = null) ; les tracés Visio des autres flèches sont intacts. Non corrigeables (listés seulement) : `isolated`, `renvoi`, `duplicate`. Testé (`tests/carto` — script `test_autofix.js` + intégration navigateur).
