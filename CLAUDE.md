...
- **Losanges décoratifs** (non connectés, posés « sur » une flèche dans Visio sans `<Connect>`) : `spliceDecisions` DÉSACTIVÉ (les insérer dans le flux complexifiait les flèches pour rien). En classique les tracés Visio sont gardés tels quels → les losanges restent posés sur LEUR flèche, rien à re-seater.
