...
- **Exclusivité des points de connexion (un point = un seul branchement)** :
  - Formes normales : `spreadPort` répartit via `unifiedUsage` (entrantes **+** sortantes) hors agencement auto. En agencement auto, `autoAssignPorts` pose un `t` explicite qui court-circuite `unifiedUsage` → une **passe finale** garantit l'exclusivité et **aligne chaque port sur la projection de sa cible** (flèche presque droite vers sa cible → moins de croisements près des formes ; groupes compris), avec écart minimal pour ne pas se superposer.
  - Losanges : les 4 pointes sont des **pins libavoid EXCLUSIFS** (`setExclusive(true)`) ET une **passe de dédoublonnage interne** (`_TIP_FALLBACK`) assigne une pointe distincte à chaque flèche côté routeur interne. Deux branches (Oui/Non) partent de pointes différentes. Au-delà de 4 connexions (rare), dégradation propre.
- **Pointes de flèche (2 protections cumulées, quel que soit le moteur)** :
  - `polylineToPath(pts, R, tipPad=18)` borne le rayon du **dernier coude** → approche **droite ≥ 18 px** avant la tête : la pointe ne se pose jamais sur un virage.
  - `_alignPortApproach()` (au rendu, visuel only) repositionne le dernier coude pour que le **dernier segment suive l'axe du port** → fini les têtes qui « pivotent » de 90° quand le port recalculé ne coïncide pas avec la fin du tracé (mesuré sur hard.vsdx : 36 → 1 pointe désalignée).
- **Losanges de décision** — DEUX cas selon la connectivité :
  - **Connectés** : recentrés en pré-routage via `_alignDecisionsToNeighbors()` (voisins connectés, borné 80 px). ⚠️ **Jamais déplacés après routage** (userPts figés → désaxage). Flèches accrochées pile aux pointes.
  - **Décoratifs (non connectés)** : dans Visio certains losanges sont posés « sur » un connecteur sans `<Connect>`. **`spliceDecisions` est DÉSACTIVÉ** (opt-in seulement) : l'insérer dans le flux (`A→B` → `A→D→B`) rajoutait un nœud sur chaque chemin et **complexifiait les flèches** pour un gain faible. Comportement retenu : **`_tagFloatingDecisions()` à l'import** (customPath encore présent) associe chaque losange décoratif à SA flèche d'origine (`_seatConnId` + position fractionnaire `_seatFrac`, via le tracé Visio), puis **`_seatFloatingDecisions()` (post-routage) le repose sur CETTE flèche, à la même position** — plus « la flèche la plus proche » (qui après re-routage était souvent la MAUVAISE : mesuré 11/19 losanges sur la mauvaise flèche → 0 avec le tag). Repli sur la flèche la plus proche uniquement pour les losanges vraiment flottants dans Visio (>60 px de toute flèche). Validé par le banc `tests/carto/` (métrique `deco`).
- **Ordre du pipeline `_computeAutoLayout` (anti-conflit)** : 1) `_alignConnectedShapes()` redresse les formes quasi-alignées (union-find transitif : colonnes = X commun, lignes = Y commun, médiane) ; 2) `_declutterShapes()` aère en poussant SUR L'AXE LIBRE → **préserve** l'alignement de l'étape 1 ; 3) `_alignDecisionsToNeighbors()` (losanges connectés) ; 4) routage ; 5) `_seatFloatingDecisions()` ; 6) `_straightenTips()` + `architectLabels()`. Étapes 1-2 débrayables via le toggle « Aérer les formes ».
- **Import VSDX** : à la reconstruction, l'utilisateur **choisit** (`_askVsdxLayoutMode`) entre **Agencement automatique** (`_computeAutoLayout()`, routage propre — **abandonné, ne plus développer**) et **Reconstruction classique** (fidèle Visio : les `customPath` exacts deviennent les `userPts` rendus). `spliceDecisions` est désactivé (les losanges décoratifs sont seatés post-routage, pas insérés dans le flux).
  - **Retouche classique (`_reconstructClassicPolish()`)** : la reconstruction classique NE ré-agence PAS mais nettoie deux choses (mesuré sur hard.vsdx) : (1) **angles droits** — `_orthogonalizeStaircase()` orthogonalise chaque tracé Visio quasi-droit (union-find : points reliés par un segment vertical partagent X, horizontal partagent Y ; valeur ancrée aux ports, sinon médiane des coudes) → 209 segments biaisés → 21 (le reste = vraies diagonales Visio) ; (2) **labels** — `architectLabels()` (récupéré de l'agencement auto) place les labels près des pointes SANS jamais les poser là où une flèche en croise une autre → 222/243 labels placés, 0 sur une autre flèche. Validé par le banc `tests/carto/` (`?classic=1&fix=1`, métriques `angles`/`labels`).
- `_showLayoutLoading()` : **animation thématique** (mini-carto qui se route : nœuds qui respirent + impulsions colorées le long des flèches, couleurs OPTIQ, `prefers-reduced-motion` respecté) + **chrono** + message « grande cartographie » ; aperçu **avant/après** avec Appliquer/Annuler + toggle d'aération (bouton uniquement ; l'import VSDX applique le mode choisi directement).
- ⚠️ **libavoid inexploitable sur les grosses cartos DENSES** (mesuré : hard.vsdx = 165 formes / 243 flèches → **>90 s même en config rapide**, le routage de base est le goulot) → au-delà du timeout c'est le **routeur interne** qui prend le relais. C'est donc lui qu'on fiabilise pour les grosses cartos réelles.
